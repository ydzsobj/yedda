...
//            dd($result_arr);
    ...
    public static function attr_sku_product($goods_kind_id)
    {
        $goods_kind = goods_kind::find($goods_kind_id);
        $good_kind_sku = $goods_kind->goods_kind_sku;
        $skuSDK = new skuSDK($goods_kind_id, $goods_kind->goods_product_id, $goods_kind->goods_kind_user_type);
        $config_sort = $skuSDK->get_attr_sku_config_sort();
        $kind_configs = kind_config::with('vals')->where('kind_primary_id', $goods_kind_id)->get()->toArray();
        $kind_sku = kind_sku::where('goods_kind_id',$goods_kind_id)->get()->toArray();
        $sku_arr = [];
        foreach ($kind_sku as $value){
            $sku_arr[$value['kind_sku']] = $value['num'];
        }
        $result_arr = [];
        if ($kind_configs) {
            $first_configs = array_shift($kind_configs);
...
//            dd($result_arr);
        ...
        if($result_arr){
            foreach ($result_arr as &$value) {
                $value['sku'] = '';
                $value['val'] = '';
                foreach ($config_sort as $k=>$item) {
                    if(isset($value[$item])){
                        $value['sku'] .= $value[$item]['kind_val_sku'];
                        $value['sku_'.$k] = $value[$item]['kind_val_id'];
                        $value['val'] .= $value[$item]['kind_val_msg'] . ',';
                    }else{
                        $value['sku_'.$k] = 0;
                        $value['sku'] .= '00';
                ...
                $value['sku'] = $good_kind_sku.$value['sku'];

                if(isset($sku_arr[$value['sku']])) {
                    $value['num'] = $sku_arr[$value['sku']];
                }else{
                    $value['num'] = 0;
                }
            }
