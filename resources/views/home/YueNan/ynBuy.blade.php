...
<html>
    <head>
        ...
        <script src="/js/diqu/yuenan.js"></script>
...
<div id="orderlog" class="Popup">
        <div>
            <div>
                <h3>Lệnh xác nhận.</h3><img style="top:5px;right:5px;position: absolute;z-index: 9;padding-left: 10px; width: 35px;" src="/img/close.png" onclick="(function(){$('#orderlog').hide()})()">
                <div id="orderlogConten">
                </div>
                <div id="orderlogConten2"></div>

            </div>
            <button id="payOk" style="width:60%;color:white;background-color:red;position: absolute;margin-left: 20%;bottom: 0px;">Xác nhận.</button>
        </div>
    </div>
...
<div class="mui-input-group">
    ...
    <div class="mui-input-row">
        <label><span class="require">*</span>tên họ:</label>
        <input type="text" name="firstname" datatype="s1-30" placeholder="phải điền, điền vào tên họ của người nhận hàng" nullmsg="" class="mui-input-clear">
    </div>
    ...
    <div class="mui-input-row">
        <label><span class="require">*</span>lựa chọn khu vực:</label>
        <div id="twzipcode"></div>
    </div>
    <div class="mui-input-row">
        <label><span class="require">*</span>địa chỉ kỹ:</label>
        <input type="text" datatype="z1-300" placeholder="phải điền, thông tin đường phố, biển số nhà" nullmsg="" errormsg="" name="address1" class="mui-input-clear">
    </div>
    <div class="mui-input-row" style="display:none;">
    ...
    <div class="mui-input-row" style=" height:66px">
        <label>dặn dò:</label>
        <textarea name="notes" placeholder="chọn điền, ví dụ điện thoại công cộng, quy cách sản phẩm hoặc thời gian phân phối hàng"></textarea>
    </div>
    ...
    <ul>
        ...
        <li>
          ...
          <div class="mui-input-row mui-radio mui-left cash-on-delivery" style="display: inline-block">
            ...
            <label>
            Thanh toán sau khi nhận hàng         </label>
...
var payFun=function (){
    ...
    if(datasObj.address1==null||datasObj.address1==''){
        layer.msg('địa chỉ kỹ không được làm chỗ trống');
        return false;
    }
    if(datasObj.city==null||datasObj.city==''){
        layer.msg('xin chọn thông tin khu vực ');
        return false;
    }
    if(datasObj.firstname==null||datasObj.firstname==''){
        layer.msg('xin điền vào tên họ của người nhận hàng ');
        return false;
    }
    if(datasObj.telephone==null||datasObj.telephone==''){
        layer.msg('xin điền vào số điện thoại của người nhận hàng');
        return false;
    }
...
var payFunGo= function (){
    ...
    if(issubmit){
        ...
        if(payType==1){
           ...
           error: function(data) {
               layer.close(index);
               layer.msg('đưa ra đơn đặt hàng thất bại, xin kiểm tra tình trạng mạng lưới');
           }
        }) ; 
        }else{
            ...
            $.ajax({
                ...
                success: function (data) {
                    layer.close(index);
                    if(data.err=='0'){
                        layer.msg('Thanh toán qua Paypal không thành công, vui lòng chọn phương thức thanh toán khác！');
                         issubmit=true;
                    }else{
                        var btime=getNowDate();
                        try{fbq('track', 'InitiateCheckout')}catch(e){};
        ...
        layer.close(index);
        layer.msg('đơn đặt hàng đã đưa ra,dừng đưa lại đơn đặt hàng');
    }
                    ...
                    if(msg.goods.goods_cuxiao_type=="0"){
                         $(function(){
	                        ...
	                        $('#addcart-quantity-dec').bind('click',function(){
	                        	...
	                        	$(this).next().val(num-1);
	                        	$('.addcart-specs-title-name').html("tổng số lượng："+(num-1));
                        ...
                        $(function(){
		                   ...
		                   var price=pricehtml.html().replace(/[^0-9]/ig,"");
	                       $('#addcart-quantity-dec').bind('click',function(){
		                   layer.msg('Mục này chỉ hỗ trợ mua gói');
		                   return false;
	                       })
	                       $('#addcart-quantity-inc').bind('click',function(){
                            ...
                            $('.addcart-float-buttons-block').children('button').click(function(){    	
                            	...
                            	if(attr=='chose_cart'){
                                   ...
                                   $('div.unfold .addcart-specs-title-name').html("tổng số lượng："+num);
		                        console.log(num);
